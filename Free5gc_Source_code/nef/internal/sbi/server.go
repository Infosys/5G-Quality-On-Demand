...
func NewServer(nef nef, tlsKeyLogPath string) (*Server, error) {
	...
	endpoints = s.getQoSEndpoints()
	group = s.router.Group(factory.QosResUriPrefix)
	applyEndpoints(group, endpoints)

	endpoints = s.getOauthEndpoints()
	group = s.router.Group(factory.NefOuthResUriPrefix)
	applyEndpoints(group, endpoints)

	endpoints = s.getAsSessionWithQoSEndpoints()
	group = s.router.Group(factory.AsSessionWithQoSResUriPrefix)
	
	group.Use(func(c *gin.Context) {
		util.Check(c, s.Context())
	})
	applyEndpoints(group, endpoints)
...
func (s *Server) startServer(wg *sync.WaitGroup) {
	...
	if scheme == "http" {
		...
		fmt.Printf("SBI server TLS cert: %s, key: %s\n", s.Config().TLSPemPath(), s.Config().TLSKeyPath())
